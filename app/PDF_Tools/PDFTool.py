...
# endregion
    ...
    def PDFCropTab(parent):
        ...
        ttk.Entry(frm, textvariable=parent.pdf_in_var, state='readonly', width=50).grid(row=0, column=0)
        ttk.Button(frm, text='Browse PDF(s)', command=lambda: _browse_pdfs(parent)).grid(row=0, column=1)
        parent.pdf_save_var = tk.StringVar()
        ttk.Entry(frm, textvariable=parent.pdf_save_var, state='readonly', width=50).grid(row=1, column=0)
    ...
    root = tk.Tk() #equilux, if you can figure out how to make the title bar dark
    root.tk.call('package', 'require', 'tkdnd')

    root.title("PDF Utility App")
    root.geometry("650x250")
    root.iconbitmap("pdf.ico")
    place_on_active_monitor(root, 650, 250)
